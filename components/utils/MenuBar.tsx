...
const MenuBar: React.FC<MenuBarProps> = ({ isOpened, showSection, hideSection, currentSection, isRight=false }) => {
    ...
    return (
        <div className={`absolute top-0 bottom-0 w-[47.5%] bg-[#061012] transition-all pt-11 px-28 ${isOpened ? 'flex' : 'hidden'} ${isRight ? 'right-0' : 'left-0'}`}>
            <div className={`w-full h-full flex flex-col justify-between z-10 font-poppins font-[600] text-[20px] leading-[30px] text-white ${isRight ? 'items-end' : 'items-start'}`}>
                <div className="relative px-6 py-2 cursor-pointer flex items-center" onClick={() => navigateSection("LABS")}>
                    <p className="relative z-10 mt-1">Launch a CTO</p>
                    <svg width="100%" className="absolute top-0 left-0" viewBox="0 0 149 43" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M149 0.772217V23.2333L129.944 42.2279H0V19.681L19.1571 0.772217L149 0.772217Z" fill="url(#paint0_linear_473_35976)" />
                        <defs>
                            <linearGradient id="paint0_linear_473_35976" x1="-168.226" y1="-84.2419" x2="131.877" y2="48.2359" gradientUnits="userSpaceOnUse">
                                <stop stopColor="white" />
                                <stop offset="1" stopColor="#FA0B0F" />
                            </linearGradient>
                        </defs>
                    </svg>
                </div>
                <div className="px-2 cursor-pointer" onClick={() => navigateSection("WELCOME")}>V Say Welcome Home</div>
                <div className="px-2 cursor-pointer" onClick={() => navigateSection("GOVERNANCE")}>CTO Governance</div>
                <div className="px-2 cursor-pointer" onClick={() => navigateSection("AIRDROP")}>Airdrop Guild</div>
                <div className="px-2 cursor-pointer" onClick={() => navigateSection("STAKING")}>Staking Forge</div>
                <div className="px-2 cursor-pointer" onClick={() => navigateSection("BRIDGE")}>Bridge</div>
                <div className="px-2 cursor-pointer" onClick={() => navigateSection("GOVERNANCE")}>RBV Governance</div>
                <div className="px-2 cursor-pointer" onClick={() => navigateSection("TASKBOARD")}>Task Board/Suggesction</div>
                <div className="px-2 pb-2 cursor-pointer" onClick={() => navigateSection("RBV")}>RBV Token</div>
            ...
            <svg width="100%" height="100%" className="absolute top-0 left-0 -z-0" xmlns="http://www.w3.org/2000/svg">
                <line x1="0%" y1="86%" x2="100%" y2="86%" stroke="#FFFFFF4D" strokeWidth={1} />
                <line x1={isRight ? "10%" : "90%"} y1="0%" x2={isRight ? "10%" : "90%"} y2="100%" stroke="#FFFFFF4D" strokeWidth={0.8} />
            </svg>
        </div>
    )
}
